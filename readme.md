...
# Build all configurations (same as default)
python3 build.py --build-all

# Build personal config (default — trackball+tps43, both sides)
python3 build.py
```
...
- Install Python deps: `python3 -m pip install -r requirements-dev.txt`
- Add the local CLI to your PATH: `export PATH="$PWD/bin:$PATH" && export ORIG_CWD="$PWD" && export PYTHONPATH="$PWD/lib/python"`
- Run `flake8 build.py tools/callgraph.py tests/test_build_py.py` and `python3 -m nose2 -v` for quick validation
